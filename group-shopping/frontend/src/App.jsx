  ...
  return (
    <nav style={{ background: DD_WHITE, borderBottom: `1px solid ${DD_BORDER}`, height: 64, display: "flex", alignItems: "center", padding: "0 24px", gap: 32, position: "sticky", top: 0, zIndex: 300, boxShadow: "0 1px 6px rgba(0,0,0,.07)" }}>
      <div onClick={onLogoClick} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", flexShrink: 0 }}>
        <div style={{ width: 36, height: 36, background: `linear-gradient(135deg,#e8734a,${DD_RED})`, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(192,57,43,.3)" }}>
...
function PrimaryBtn({ children, onClick, disabled, fullWidth, style: s = {} }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{ background: disabled ? "#e0e0e0" : `linear-gradient(135deg,#e8734a,${DD_RED})`, color: disabled ? "#aaa" : "#fff", border: "none", borderRadius: 10, padding: "13px 24px", fontWeight: 700, fontSize: 14, boxShadow: disabled ? "none" : "0 4px 16px rgba(192,57,43,.3)", width: fullWidth ? "100%" : "auto", letterSpacing: "0.01em", ...s }}>
      {children}
    </button>
  );
}
...
function OutlineBtn({ children, onClick, style: s = {} }) {
  return (
    <button onClick={onClick} style={{ background: DD_WHITE, color: DD_RED, border: `2px solid ${DD_RED}`, borderRadius: 10, padding: "11px 24px", fontWeight: 700, fontSize: 14, ...s }}>
      {children}
    </button>
  );
}
    ...
    >
      ...
      <div style={{ padding: "10px 12px 14px" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={() => onShare(p)} style={{ flex: 1, background: DD_WHITE, border: `1.5px solid ${DD_BORDER}`, color: DD_GRAY, borderRadius: 8, padding: "7px 0", fontSize: 11, fontWeight: 600 }}>💬 Share</button>
          <button onClick={() => onCart(p)} style={{ flex: 2, background: inCart ? "#27ae6014" : `linear-gradient(135deg,#e8734a,${DD_RED})`, border: inCart ? "1.5px solid #27ae60" : "none", color: inCart ? "#27ae60" : "#fff", borderRadius: 8, padding: "7px 0", fontSize: 11, fontWeight: 700 }}>
...
function ChatProduct({ p, onCart, inCart }) {
  return (
    <div style={{ background: DD_WHITE, border: `1.5px solid ${DD_BORDER}`, borderRadius: 12, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10, marginTop: 6, maxWidth: 290, boxShadow: "0 2px 8px rgba(0,0,0,.07)" }}>
      ...
      <button onClick={() => onCart(p)} style={{ background: inCart ? "#27ae6014" : `linear-gradient(135deg,#e8734a,${DD_RED})`, border: inCart ? "1.5px solid #27ae60" : "none", color: inCart ? "#27ae60" : "#fff", borderRadius: 8, padding: "8px 11px", fontSize: 11, fontWeight: 700, flexShrink: 0, whiteSpace: "nowrap" }}>
        {inCart ? "✓ Added" : "+ Cart"}
      </button>
    </div>
  );
...
export default function App() {
  ...
  /* ── helpers ── */
  const goHome = () => {
    if (room) socket.emit("leave-room", { roomId: room.id, userName: myName });
    socket.disconnect();
  ...
  const doJoin = async () => {
    ...
    setLoading(true); setError(""); setJoinErr("");
    try {
      const data = await api.getRoomByCode(joinCode.trim());
      if (data.members.length >= data.maxMembers && !data.members.find(m => m.name === myName)) {
        setJoinErr("This room is full."); return;
      }
      enterRoom(data);
    } catch (e) { setJoinErr("Room not found. Check the code."); }
    finally { setLoading(false); }
  };
    ...
    <div style={{ minHeight: "100vh", background: DD_LIGHT }}>
      ...
      <div style={{ background: "linear-gradient(120deg,#fff5f2 0%,#fff 60%,#fff8f5 100%)", borderBottom: `1px solid ${DD_BORDER}`, padding: "52px 24px 48px", textAlign: "center" }}>
        <div style={{ maxWidth: 580, margin: "0 auto" }}>
          ...
          <p style={{ color: DD_GRAY, fontSize: 15, lineHeight: 1.75, marginBottom: 32 }}>Create a group room, invite friends, share products in chat<br />and build a shared cart — all in real time.</p>
          ...
          {error && <div style={{ color: DD_RED, marginBottom: 12, fontSize: 13 }}>{error}</div>}
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "32px 20px 60px" }}>
        ...
        <div style={{ background: DD_WHITE, borderRadius: 16, padding: 26, boxShadow: "0 4px 18px rgba(0,0,0,.07)", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 22 }}>
            {THEMES.map(t => (
              <button key={t.label} onClick={() => setCForm(f => ({ ...f, theme: t }))} style={{ padding: "8px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, border: `1.5px solid ${cForm.theme.label === t.label ? t.accent : DD_BORDER}`, background: cForm.theme.label === t.label ? t.accent + "14" : "#fafafa", color: cForm.theme.label === t.label ? t.accent : DD_GRAY, transition: "all .2s" }}>{t.emoji} {t.label}</button>
            ))}
          </div>
    ...
    <div style={{ minHeight: "100vh", background: DD_LIGHT }}>
      ...
      <div style={{ maxWidth: 440, margin: "0 auto", padding: "32px 20px 60px" }}>
        ...
        <div style={{ background: DD_WHITE, borderRadius: 16, padding: 26, boxShadow: "0 4px 18px rgba(0,0,0,.07)", marginBottom: 16 }}>
          ...
          <input value={joinCode} maxLength={6} onChange={e => { setJoinCode(e.target.value.toUpperCase().slice(0,6)); setJoinErr(""); }} onKeyDown={e => e.key === "Enter" && doJoin()} placeholder="AB1C2D" style={{ width: "100%", border: `1.5px solid ${DD_BORDER}`, borderRadius: 10, padding: "16px", fontSize: 30, fontWeight: 900, letterSpacing: "0.35em", textAlign: "center", color: DD_DARK, background: "#fafafa" }} />
      ...
      <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: DD_LIGHT, overflow: "hidden" }}>
        ...
        <div style={{ background: DD_WHITE, borderBottom: `1.5px solid ${DD_BORDER}`, padding: "10px 16px", flexShrink: 0, boxShadow: "0 1px 6px rgba(0,0,0,.05)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <button onClick={() => { navigator.clipboard.writeText(room.code).catch(() => {}); setCopied(true); setTimeout(() => setCopied(false), 2000); }} style={{ background: copied ? "#27ae6012" : `${DD_RED}10`, border: `1.5px solid ${copied ? "#27ae60" : DD_RED + "44"}`, color: copied ? "#27ae60" : DD_RED, borderRadius: 8, padding: "7px 16px", fontSize: 11, fontWeight: 700 }}>
          ...
          <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 2 }}>
            {room.members.map(m => (
              <div key={m.name} style={{ display: "flex", alignItems: "center", gap: 5, whiteSpace: "nowrap", flexShrink: 0, background: m.name === myName ? `${accent}12` : "#f8f8f8", border: `1.5px solid ${m.name === myName ? accent + "44" : DD_BORDER}`, borderRadius: 20, padding: "4px 12px", fontSize: 11, fontWeight: 600 }}>
                <span>{m.avatar}</span><span style={{ color: m.name === myName ? accent : DD_DARK }}>{m.name}</span>
                {m.name === room.host && <span style={{ fontSize: 8, color: "#e67e22", fontWeight: 900 }}>HOST</span>}
              </div>
            ))}
            {mc < room.maxMembers && <div style={{ display: "flex", alignItems: "center", whiteSpace: "nowrap", background: "#fafafa", border: `1.5px dashed ${DD_BORDER}`, borderRadius: 20, padding: "4px 12px", fontSize: 11, color: "#ccc", flexShrink: 0 }}>+{room.maxMembers - mc} open</div>}
          </div>
        </div>
        ...
        <div style={{ background: DD_WHITE, display: "flex", borderBottom: `1.5px solid ${DD_BORDER}`, flexShrink: 0 }}>
          {[{ id: "chat", label: "💬 Chat" }, { id: "shop", label: "🛍 Shop" }, { id: "cart", label: `🛒 Cart${room.cart.length > 0 ? ` (${room.cart.length})` : ""}` }].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, background: "none", border: "none", borderBottom: `3px solid ${tab === t.id ? DD_RED : "transparent"}`, color: tab === t.id ? DD_RED : DD_GRAY, padding: "13px 0", fontSize: 13, fontWeight: 700, transition: "all .2s" }}>{t.label}</button>
          ))}
        </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, background: "#f9f9f9" }}>
                ...
                return (
                  <div key={msg.id} className="msg" style={{ display: "flex", flexDirection: me ? "row-reverse" : "row", alignItems: "flex-end", gap: 8, marginBottom: 4 }}>
                    ...
                    <div style={{ maxWidth: "72%", display: "flex", flexDirection: "column", alignItems: me ? "flex-end" : "flex-start" }}>
                      ...
                      {msg.type === "text" && (
                        <div style={{ background: me ? `linear-gradient(135deg,#e8734a,${DD_RED})` : DD_WHITE, color: me ? "#fff" : DD_DARK, border: me ? "none" : `1.5px solid ${DD_BORDER}`, borderRadius: me ? "16px 16px 4px 16px" : "16px 16px 16px 4px", padding: "10px 14px", fontSize: 13, lineHeight: 1.55, wordBreak: "break-word", boxShadow: me ? "0 2px 10px rgba(192,57,43,.22)" : "0 1px 4px rgba(0,0,0,.06)" }}>{msg.text}</div>
                      )}
                      {msg.type === "product" && (
                        <div style={{ display: "flex", flexDirection: "column", alignItems: me ? "flex-end" : "flex-start" }}>
                          <div style={{ background: me ? `linear-gradient(135deg,#e8734a,${DD_RED})` : DD_WHITE, color: me ? "#fff" : DD_DARK, border: me ? "none" : `1.5px solid ${DD_BORDER}`, borderRadius: "16px 16px 16px 4px", padding: "8px 14px", fontSize: 12, lineHeight: 1.4, boxShadow: me ? "0 2px 10px rgba(192,57,43,.22)" : "0 1px 4px rgba(0,0,0,.06)" }}>
            ...
            <div style={{ flexShrink: 0, padding: "10px 12px", background: DD_WHITE, borderTop: `1.5px solid ${DD_BORDER}`, position: "relative" }} ref={pickerRef}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <button onClick={() => setShowPicker(v => !v)} title="Share a product in chat" style={{ width: 40, height: 40, borderRadius: 10, flexShrink: 0, border: `1.5px solid ${showPicker ? DD_RED : DD_BORDER}`, background: showPicker ? `${DD_RED}10` : "#fafafa", color: showPicker ? DD_RED : DD_GRAY, fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>🛍</button>
                ...
                <button onClick={sendText} disabled={!chatTxt.trim()} style={{ width: 40, height: 40, borderRadius: 10, border: "none", flexShrink: 0, background: chatTxt.trim() ? `linear-gradient(135deg,#e8734a,${DD_RED})` : DD_BORDER, color: chatTxt.trim() ? "#fff" : "#bbb", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: chatTxt.trim() ? "0 2px 10px rgba(192,57,43,.25)" : "none" }}>↑</button>
              </div>
        ...
        {tab === "shop" && (
          <div style={{ flex: 1, overflowY: "auto", background: DD_LIGHT }}>
            <div style={{ padding: "12px 14px 8px", fontSize: 11, fontWeight: 700, color: DD_GRAY, letterSpacing: "0.07em", textTransform: "uppercase" }}>Tap "Share" to send a product to the group chat</div>
